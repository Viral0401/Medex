...
const GovScehems = () => {
  ...
  return (
    <div>
      <p className='text-4xl px-10 pt-10'>Stay Ahead: Access Government Schemes</p>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 p-5'>
        {data.slice(0, visibleItems).map((schemeData, index) => (
          <div key={index} className='border-[1px] border-gray-100 shadow-2xl m-3 p-5 rounded-2xl transition duration-500 hover:-translate-y-2 flex flex-col items-center justify-between'>
            <div className='mb-4'>
              <img src={schemeData.logo} alt={schemeData.title} className='h-[200px] w-full rounded-2xl shadow-xl' />
              <p className='py-2 text-2xl font-bold text-[#1b1b1b] w-full'>{schemeData.title}</p>
              <p className='py-2 text-sm tracking-widest font-normal text-[#1b1b1b] w-full'>{schemeData.content}</p>
            </div>
